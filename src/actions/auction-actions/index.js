...
export const isNftOnAuctionETH = async (memeId) => {
  try {
...
export const auctionNft = ({ memeRefId, memeId, startingPrice, endingPrice, endingAuctionTime }) => async (dispatch) => {
  try {
    dispatch({
      type: CREATE_NFT_AUCTION_LOADING,
      payload: {},
    });
    ...
    const onAuction = await isNftOnAuctionETH(memeId);
    if (onAuction) {
    ...
    await NftsChainCore.methods.createSaleAuction(
      memeId,
    ...
    dispatch({
      type: CREATE_NFT_AUCTION_SUCCESS,
      payload: {},
    });
...
export const cancelAuction = (memeId) => async (dispatch) => {
  dispatch({
    type: CANCEL_NFT_AUCTION_LOADING,
    payload: {},
  });
  ...
  try {
    ...
    dispatch({
      type: CANCEL_NFT_AUCTION_SUCCESS,
      payload: {},
    });

  } catch ({ message = '' }) {
    dispatch({
      type: CANCEL_NFT_AUCTION_ERROR,
      payload: {
...
export const buyNft = ({ auction, memeId }) => async (dispatch) => {
  dispatch({
    type: BUY_NFT_FROM_AUCTION_LOADING,
    payload: {},
  });
  ...
  try {
    ...
    dispatch({
      type: BUY_NFT_FROM_AUCTION_SUCCESS,
      payload: {},
    });
  } catch ({ message = '' }) {
    dispatch({
      type: BUY_NFT_FROM_AUCTION_ERROR,
      payload: {
        message: message.split('\n')[0],
      },
    });
