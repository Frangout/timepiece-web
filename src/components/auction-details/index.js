...
const AuctionCreateForm = (props) => {
    const { visible, onCancel, onCreate, form, auctionPrice, createNftAuction } = props;
    ...
    return (
      <Modal
        ...
        onOk={onCreate}
        onCancel={onCancel}
        confirmLoading={createNftAuction.loading}
      >
...
class AuctionDetails extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const { auctionPrice, createNftAuction, cancelNftAuction, buyNftFromAuction } = nextProps;
    if (auctionPrice.errors.error) {
      message.error(`Could not get auction price, ${auctionPrice.errors.message}`);
    }
    if (createNftAuction.errors.error) {
      message.error(createNftAuction.errors.message);
    }
    if (createNftAuction.success.ok) {
      // this.form.resetFields();
      this.setState({ modalVisible: false });
      window.location.reload();
    }

    if (cancelNftAuction.errors.error) {
      message.error(cancelNftAuction.errors.message);
    }

    if (cancelNftAuction.success.ok) {
      message.info('Your meme is out of auction.');
      window.location.reload();
    }

    if (buyNftFromAuction.success.ok) {
      message.success('You now own this meme! Cheers!');
      setTimeout(() => window.location.reload(), 2000);
    }

    if (buyNftFromAuction.errors.error) {
      message.error(buyNftFromAuction.errors.message);
    }
  }
  ...
  async buyNft() {
    this.props.buyNft(this.props.meme);
  }
    ...
    if (this.props.currentUser && (this.props.meme.owner.username === this.props.currentUser.username)) { // User is owner of meme
      ...
      if (!this.props.meme.auction) {
        return (
          <Button
            ...
            onClick={this.sellNft.bind(this)}
      ...
      return (
        <Button
          ...
          loading={this.props.cancelNftAuction.loading}
        >
          Cancel Auction
        </Button>
      );
      ...
      if (this.props.currentUser) {
        ...
        //         type="primary"
        //         className="app-primary-large"
        //         onClick={this.buyNft.bind(this)}
        //         loading={this.props.buyNftFromAuction.loading}
        //       >
        //         Buy
        //       </Button>
        //     </Popover>
    ...
    return (
      <div className="auctions-component-wrapper">
        ...
        <AuctionCreateForm
          ...
          createNftAuction={this.props.createNftAuction}
        />
      </div>
    );
  }
...
const mapStateToProps = ({ parseReducers, auctionReducers }) => {
  ...
  const { auctionPrice, createNftAuction, cancelNftAuction, buyNftFromAuction } = auctionReducers;
  return { currentUser, auctionPrice, createNftAuction, cancelNftAuction, buyNftFromAuction };
};
